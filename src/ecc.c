...
void ecc_init(ecc curve, const char* a_str, const char* b_str, const char* p_str)
{
    ...
    assertf(
        ...
        "Intialize curve failed: curve->p must be a prime number!"
    );
...
void ecc_init_pt_str(ecc curve, eccpt point, const char* x_str, const char* y_str, const char* z_str)
{
    ...
    assertf(
        ...
        "Verify point failed: Point is not on curve." 
    );
}
...
{
    assertf(ecc_verify_pt(curve, pointP), "Cannot perform P+Q if P is not on curve.");
...
{
    assertf(ecc_verify_pt(curve, pointP), "Cannot compare P == Q if P is not on curve.");
    assertf(ecc_verify_pt(curve, pointQ), "Cannot compare P == Q if Q is not on curve.");
    assertf(mpz_cmp_si(pointP->y, UNDEFINED_COORDINATE) != 0, "Cannot compare P == Q if P.y is undefined.");
    assertf(mpz_cmp_si(pointQ->y, UNDEFINED_COORDINATE) != 0, "Cannot compare P == Q if Q.y is undefined.");
...
{
    assertf(ecc_verify_pt(curve, pointP), "Cannot compute Weil Pairing e(P, Q) if P is not on curve!");
    assertf(ecc_verify_pt(curve, pointQ), "Cannot compute Weil Pairing e(P, Q) if Q is not on curve!");
    assertf(mpz_cmp_si(n, 0) > 0, "Please provide positive n to compute Weil Pairing!");
