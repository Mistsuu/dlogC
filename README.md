...
This problem can be solved if we only hash points that satisfy some constraints that we created *(in the paper they're called the "distinguished points")*, and check for collisions in those cases only. While at first glance, it seems like many possible equal values will be missed, but trust me, the math checks out. In this implementation, to determine if a point is a distinguished point, I just check whether the first limb of the point's X-coordinate modulo some number equals 0. *(yeah that's it)*